...
In addition to the features already listed for Prism, Qsmart also gives the "Rides+" functionality, which allows choosing among a different set of ride packages, control visitors' ride frequency and so on.
Finally, the ride reservation is validated by a QR code.
Finally, the company states that its mobile payments are PCI\footnote\url{{https://en.wikipedia.org/wiki/Payment\_Card\_Industry\_Data\_Security\_Standard}} compliant.
