...
direction and travel distance. The current location can be calculated from the initial location \texttt{P}. The distance and direction can
...
We use $f = {r_1, r_2, \ldots, r_n}$ to indicate a \textit{fingerprint} where $r_i$ represents the RSS value of captured AP and $n$ the number of APs
in the fingerprint.
We calculate the dissimilarity between two fingerprints based on RSS difference. Denote with $\sigma_i = | r_i - r^{'}_{i}|$ the difference of
fingerprints $f^{'}$ and $f$ at each $A_i$ where $A_i \in A$ where $A$ is the fingerprint's APs set.
...
(more accurate location and lower consumption) and costs.
